const randomUUID=()=>Date.now().toString(36)+'-'+Math.random().toString(36).slice(2);
const fingers=['拇指','食指','中指','无名指','小指'];
const emptyData=()=>({reports:[],templates:[],calibration:null,settings:{voice:true,tolerance:100,learner:'陶艺学习者',baudRate:115200,serviceUUID:'',notifyUUID:'',writeUUID:''}});
const validValues=v=>Array.isArray(v)&&v.length===5&&v.every(n=>typeof n==='number'&&Number.isFinite(n)&&n>=0&&n<=65535);
...
class Studio {
 constructor(data=emptyData(),now=Date.now){this.data={...emptyData(),...data,settings:{...emptyData().settings,...data.settings}};this.now=now;this.device={connected:false,name:'陶艺手套',source:'',values:null,lastSeen:0};this.session=null;this.samples=[];this.events=[];this.peers={};this.feedback={kind:'waiting',text:'等待手套数据'};this.lastMatch=null;this.streakStart=0;this.lastCount=0;}
 ...
 ingest(values,name='陶艺手套',source='serial'){
  ...
  const match=this.data.templates.filter(x=>x.lesson==='揉泥基础').map(x=>({...x,distance:Math.max(...values.map((v,i)=>Math.abs(v-x.values[i])))})).filter(x=>x.distance<=x.tolerance).sort((a,b)=>a.distance-b.distance)[0];
 ...
 action(type,p={}){
  ...
  switch(type){
   ...
   case 'disconnect':this.device.connected=false;this.device.values=null;this.lastMatch=null;break;
   case 'calibrate':{
...
module.exports={fingers,emptyData,validValues,parseSample,Studio};
